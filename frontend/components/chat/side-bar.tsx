...
const SideBar = () => {
  ...
  const [isMessagesSeeAll, setIsMessagesSeeAll] = useState(false);
	...
	return (
		<div className="flex w-full max-w-full md:max-w-[24rem] flex-col justify-start items-center bg-bars-bg">
      ...
      : 
        <MenuIconMenu />
				...
				<div className="relative w-80">
					...
					<Input placeholder="Search" value={searchValue} onChange={(e) => setSearchValue(e.target.value)} onFocus={() => setIsSearchActive(true)} className={cn("flex h-10 pl-10 rounded-full focus:!ring-0 focus:!border-accent-primary", isSearchActive ? "border-accent-primary" : "")} />
				</div>
			</div>
            ...
            {Array.from({ length: isMessagesSeeAll ? 10 : 5 }, (_, i) => (
              <div key={i} className="w-full h-16 min-h-16 p-4 border bg-red-700 mb-2" />
				...
				{/* Chat List Panel */}
				<div className={cn("absolute inset-0 flex flex-col justify-center items-center overflow-auto transition-opacity duration-300 ease-in-out", isSearchActive ? "opacity-0 pointer-events-none" : "opacity-100")}>
          {Array.from({ length: 20 }, (_, i) => (
              <div key={i} className="w-full h-16 min-h-16 p-4 border bg-blue-700 mb-2" />
            ))}
				</div>
			</div>
		</div>
